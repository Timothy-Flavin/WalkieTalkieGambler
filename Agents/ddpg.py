...
class ddpg():
  ...
  def action(self, state, anum):
    ...
    with torch.no_grad():
      return self.policy(torch.from_numpy(sar_env.vectorize_state(state,anum,True))[None,:].to(self.device)).detach().cpu().numpy()
...
if __name__ == "__main__":
  ...
  brains = {}
  for a in agents:
    brains[a] = ddpg(st.shape[0],5,8,5,[128,64],0.95,f"./ddpg/{a}/",batch_size=32,update_every=64,eps=1,eps_decay=0.99999)
